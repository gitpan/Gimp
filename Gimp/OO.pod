...
A solution to this situation is to use OO-syntax. Gimp plays some dirty
tricks and provides a number of classes, like C<Gimp::Image> and
C<Gimp::Palette> (please note that B<all> these classes appear with the
C<Gimp::> prefix as well as without, i.e. C<Gimp::Palette> is the same class
as C<Palette>).

If you call a method, C<Gimp::OO> tries to find a gimp function by
...
Another shortcut: many functions want a (redudant) image argument, like

 $image->shear ($layer, ...)

Since you want to shear the E<$layer>, not the E<$image>, this is confusing as well.
In cases like this, Gimp allows you to write:

 $layer->shear (...)

And automatically confers the additional IMAGE-type argument.
